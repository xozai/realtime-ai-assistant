...
| Confluence export (`export_to_confluence`) | ✅ Shipped |

---

## Current Release

The README is current through **v0.20.0**. The most recent addition is the Confluence export feature (v0.20.0): `export_to_confluence` tool, `confluence_client.py`, `POST /api/confluence` dashboard endpoint, and the "Export to Confluence" dashboard button. See `CHANGELOG.md` for the full release-by-release history generated by release-please.
...
### 9. Configure Confluence export, optional

To publish discovery pages to Confluence, set:

```bash
CONFLUENCE_SPACE_KEY=your_space_key
# Optional — defaults to JIRA_BASE_URL (same Atlassian tenant)
CONFLUENCE_BASE_URL=https://your-org.atlassian.net
```

The client reuses `JIRA_USER_EMAIL` and `JIRA_API_TOKEN` for authentication. `CONFLUENCE_BASE_URL` only needs to be set when your Confluence instance is on a different subdomain from Jira.

### 10. Configure deduplication and cost rates, optional
...
curl -X POST http://localhost:8000/api/confluence
...
| `POST` | `/api/confluence` | Publish a discovery page to Confluence (requires `CONFLUENCE_SPACE_KEY`) |
...
| `export_to_confluence(title, parent_page_id, jira_issue_keys)` | Publish a Confluence page with requirements, stories, acceptance criteria, executive summary, and Jira links; creates or updates by title |
...
Publish the full discovery output to a Confluence page with:

```text
export_to_confluence()
```

Or from the dashboard using the **Export to Confluence** button, or directly:
...
- requirements table with category and confidence
- user stories with Agile statement, acceptance criteria, priority, and points
- links to Jira issues (pass `jira_issue_keys` after `submit_stories_to_jira`)

If a page with the same title already exists in the space it is updated in-place. To customize the title or nest the page:

```text
export_to_confluence(title="ACME Sprint 1 Discovery", parent_page_id="12345")
```

Requires `CONFLUENCE_SPACE_KEY` in `.env`. Reuses Jira credentials by default.
...
│       ├── confluence_client.py # ConfluenceClient (stdlib urllib, create/update pages)
...
└── tests/
    ...
    ├── test_confluence.py         # ConfluenceClient, page body builder, export_to_confluence tool
...
| `test_confluence.py` | `ConfluenceClient` HTTP (mocked), page body builder, HTML escaping, `export_to_confluence` tool |
| `test_dashboard.py` | All FastAPI endpoints via TestClient, cost/project fields, SSE update path |

**Current count: 199 passing, 0 failures.**
